ancient_5_modifier_casting = class({})

function ancient_5_modifier_casting:IsHidden() return true end
function ancient_5_modifier_casting:IsPurgable() return false end

-- CONSTRUCTORS -----------------------------------------------------------

function ancient_5_modifier_casting:OnCreated(kv)
  ...
  if IsServer() then
    AddModifier(self.parent, self.ability, "ancient_5_modifier_efx_hands", {duration = time}, false)
  end
...
function ancient_5_modifier_casting:OnRefresh(kv)
end

function ancient_5_modifier_casting:OnRemoved()
  self.parent:FadeGesture(ACT_DOTA_TELEPORT)
	self.parent:FadeGesture(ACT_DOTA_ANCESTRAL_SPIRIT)

	if IsServer() then
		self:GetParent():RemoveModifierByName("ancient_5_modifier_efx_hands")
...
function ancient_5_modifier_casting:OnIntervalThink()
	if self.step == 1 then
		self.parent:FadeGesture(ACT_DOTA_TELEPORT)
