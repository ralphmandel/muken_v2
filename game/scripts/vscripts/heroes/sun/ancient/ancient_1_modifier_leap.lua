ancient_1_modifier_leap = class({})
STATE_PRE_HIT = 1
STATE_POS_HIT = 2

function ancient_1_modifier_leap:IsHidden() return true end
function ancient_1_modifier_leap:IsPurgable() return false end

-- CONSTRUCTORS -----------------------------------------------------------

function ancient_1_modifier_leap:OnCreated(kv)
...
function ancient_1_modifier_leap:OnRefresh(kv)
end

function ancient_1_modifier_leap:OnRemoved()
...
function ancient_1_modifier_leap:CheckState()
	local state = {
...
function ancient_1_modifier_leap:DeclareFunctions()
	local funcs = {
...
function ancient_1_modifier_leap:OnStateChanged(keys)
  if keys.unit ~= self.parent then return end
  if self.parent:IsStunned() or self.parent:IsHexed() then self:Destroy() end
end

function ancient_1_modifier_leap:OnUnitMoved(keys)
  if keys.unit == self.parent then self:Destroy() end
end

function ancient_1_modifier_leap:OnIntervalThink()
...
function ancient_1_modifier_leap:PreHit()
...
function ancient_1_modifier_leap:PosHit()
  ...
  local refresh = self.parent:FindModifierByName("ancient_1_modifier_refresh")
  if IsServer() and refresh then refresh:DecrementStackCount() end
  ...
  if strike == true then PlayEfxAncientStun(self.parent, self.damage, true) end
...
function ancient_1_modifier_leap:PlayEfxHit(radius)
